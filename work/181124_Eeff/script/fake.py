...
def run_fake(arffile,exposure_ks):
	outdir = '%s/fake' % os.path.dirname(arffile)
	...
	cmd += "@work/181124_Eeff/script/crab_xmm_fixed_model.xcm\n"
	cmd += "log %s\n" % outlog
	cmd += "fakeit none\n"
	...
	cmd = 'mv %s work/181124_Eeff/effective_area/' % os.path.basename(outpdf)
	print(cmd);os.system(cmd)	
...
exposure_ks = 10.0
f = open('work/181124_Eeff/effective_area/fl_vs_rate_crab.txt','w')
