...
2018-11-05 generated in GitHub repository by Teru Enoto
"""
...
class XspecResponse():
	...
	def generate_rmffile(self,outrmffile):
		"""
		...
		# --- read ascii rmf file 
		rmf_table = '%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['RMF_TABLE'])
...
"""	% (self.DETCHANS, self.param['LO_THRES'], self.param['LO_THRES'], channel_array[0],channel_array[-1])
	...
	def generate_arffile(self,outarffile):
		"""
		this method generates XPSEC redistribution matrix file (rmf file) for NICER.
		https://heasarc.gsfc.nasa.gov/docs/heasarc/caldb/docs/memos/cal_gen_92_002/cal_gen_92_002.html#tth_sEc3.2
		"""		
		...
		xrc_snid_table = '%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['XRC_SNID_TABLE'])
		xrc_list = []
		for line in open(xrc_snid_table):
			...
			xrc_list[-1].readApertureFraction('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['XRC_APERTURE_TABLE']))
		...
		if not os.path.exists('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['QE_TABLE'])):
			sys.stderr.write("QE table %s/%s does not exist.\n" %(os.getenv('DATADIR_XSPECRESP'),self.param['QE_TABLE']))
			quit()
		qe_energy_keV_list = []
		qe_efficency_list = []
		for line in open('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['QE_TABLE'])):
			cols = line.split()
			if cols[0] == 'energy_eV':
				continue 
		...
		if not os.path.exists('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['WINDOW_TABLE'])):
			sys.stderr.write("Window transmission table %s/%s does not exist.\n" % (os.getenv('DATADIR_XSPECRESP'),self.param['WINDOW_TABLE']))
			quit()
		window_energy_keV_list = []
		window_efficiency_list = []
		for line in open('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['WINDOW_TABLE'])):
			cols = line.split()
			if cols[0] == 'Energy_eV':
				continue 
			energy_eV = float(cols[0])
		...
		if not os.path.exists('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['THERMALSD_TABLE'])):
			sys.stderr.write("Thermal shield table %s/%s does not exist.\n" % (os.getenv('DATADIR_XSPECRESP'),self.param['THERMALSD_TABLE']))
			quit()
		thermalsd_energy_keV_list = []
		thermalsd_transmission_list = []
		for line in open('%s/%s' % (os.getenv('DATADIR_XSPECRESP'),self.param['THERMALSD_TABLE'])):
			cols = line.split()
